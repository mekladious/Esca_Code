...
from train_hybrid import train_hybrid, validate_hybrid, test_hybrid
# os.environ["CUDA_VISIBLE_DEVICES"] = "4"   # select the GPU-id.
...
if __name__ == '__main__':
    ...
    parser.add_argument("-task", default='ext', type=str, choices=['ext', 'abs', 'hybrid'])
    ...
    # Hybrid means Combining the extractor with the PG-generator.
    elif (args.task == 'hybrid'):
        if (args.mode == 'train'):
            # Running the training extraction function.
            train_hybrid(args, device_id)
        elif (args.mode == 'validate'):
            # Run the validation extraction function.
            validate_hybrid(args, device_id)
        if (args.mode == 'test'):
